...
.paddingBottom);this.paddingLeft=parse(context,paddingLeft,declaration.paddingLeft);this.paintOrder=parse(context,paintOrder,declaration.paintOrder);this.position=parse(context,position,declaration.position);this.textAlign=parse(context,textAlign,declaration.textAlign);this.textDecorationColor=parse(context,textDecorationColor,(_a=declaration.textDecorationColor)!==null&&_a!==void 0?_a:declaration.color);this.textDecorationLine=parse(context,textDecorationLine,(_b=declaration.textDecorationLine)!==null&&_b!==void 0?_b:declaration.textDecoration);this.textShadow=parse(context,textShadow,declaration.textShadow);this.textTransform=parse(context,textTransform,declaration.textTransform);this.transform=parse(context,transform$1,declaration.transform);this.transformOrigin=parse(context,transformOrigin,declaration.transformOrigin);this.visibility=parse(context,visibility,declaration.visibility);this.webkitTextStrokeColor=parse(context,webkitTextStrokeColor,declaration.webkitTextStrokeColor);
this.webkitTextStrokeWidth=parse(context,webkitTextStrokeWidth,declaration.webkitTextStrokeWidth);this.wordBreak=parse(context,wordBreak,declaration.wordBreak);this.zIndex=parse(context,zIndex,declaration.zIndex);}CSSParsedDeclaration.prototype.isVisible=function(){return this.display>0&&this.opacity>0&&this.visibility===0;};CSSParsedDeclaration.prototype.isTransparent=function(){return isTransparent(this.backgroundColor);};CSSParsedDeclaration.prototype.isTransformed=function(){return this.transform!==null;};CSSParsedDeclaration.prototype.isPositioned=function(){return this.position!==0;};CSSParsedDeclaration.prototype.isPositionedWithZIndex=function(){return this.isPositioned()&&!this.zIndex.auto;};CSSParsedDeclaration.prototype.isFloating=function(){return this.float!==0;};CSSParsedDeclaration.prototype.isInlineLevel=function(){return(contains(this.display,4)||contains(this.display,33554432)||contains(this.display,268435456)||contains(this.display,536870912)||contains(this.display,
67108864)||contains(this.display,134217728));};return CSSParsedDeclaration;}());var CSSParsedPseudoDeclaration=(function(){function CSSParsedPseudoDeclaration(context,declaration){this.content=parse(context,content,declaration.content);this.quotes=parse(context,quotes,declaration.quotes);}return CSSParsedPseudoDeclaration;}());var CSSParsedCounterDeclaration=(function(){function CSSParsedCounterDeclaration(context,declaration){this.counterIncrement=parse(context,counterIncrement,declaration.counterIncrement);this.counterReset=parse(context,counterReset,declaration.counterReset);}return CSSParsedCounterDeclaration;}());var parse=function(context,descriptor,style){var tokenizer=new Tokenizer();var value=style!==null&&typeof style!=='undefined'?style.toString():descriptor.initialValue;tokenizer.write(value);var parser=new Parser(tokenizer.read());switch(descriptor.type){case 2:var token=parser.parseComponentValue();return descriptor.parse(context,isIdentToken(token)?token.value:descriptor
.initialValue);case 0:return descriptor.parse(context,parser.parseComponentValue());case 1:return descriptor.parse(context,parser.parseComponentValues());case 4:return parser.parseComponentValue();case 3:switch(descriptor.format){case'angle':return angle.parse(context,parser.parseComponentValue());case'color':return color$1.parse(context,parser.parseComponentValue());case'image':return image.parse(context,parser.parseComponentValue());case'length':var length_1=parser.parseComponentValue();return isLength(length_1)?length_1:ZERO_LENGTH;case'length-percentage':var value_1=parser.parseComponentValue();return isLengthPercentage(value_1)?value_1:ZERO_LENGTH;case'time':return time.parse(context,parser.parseComponentValue());}break;}};var elementDebuggerAttribute='data-html2canvas-debug';var getElementDebugType=function(element){var attribute=element.getAttribute(elementDebuggerAttribute);switch(attribute){case'all':return 1;case'clone':return 2;case'parse':return 3;case'render':return 4;
...
8)output+=String.fromCharCode((input[i>>5]>>>(i%32))&0xFF);return output;}function binl_md5(x,len){x[len>>5]|=0x80<<((len)%32);x[(((len+64)>>>9)<<4)+14]=len;var a=1732584193;var b=-271733879;var c=-1732584194;var d=271733878;for(var i=0;i<x.length;i+=16){var olda=a;var oldb=b;var oldc=c;var oldd=d;a=md5_ff(a,b,c,d,x[i+0],7,-680876936);d=md5_ff(d,a,b,c,x[i+1],12,-389564586);c=md5_ff(c,d,a,b,x[i+2],17,606105819);b=md5_ff(b,c,d,a,x[i+3],22,-1044525330);a=md5_ff(a,b,c,d,x[i+4],7,-176418897);d=md5_ff(d,a,b,c,x[i+5],12,1200080426);c=md5_ff(c,d,a,b,x[i+6],17,-1473231341);b=md5_ff(b,c,d,a,x[i+7],22,-45705983);a=md5_ff(a,b,c,d,x[i+8],7,1770035416);d=md5_ff(d,a,b,c,x[i+9],12,-1958414417);c=md5_ff(c,d,a,b,x[i+10],17,-42063);b=md5_ff(b,c,d,a,x[i+11],22,-1990404162);a=md5_ff(a,b,c,d,x[i+12],7,1804603682);d=md5_ff(d,a,b,c,x[i+13],12,-40341101);c=md5_ff(c,d,a,b,x[i+14],17,-1502002290);b=md5_ff(b,c,d,a,x[i+15],22,1236535329);a=md5_gg(a,b,c,d,x[i+1],5,-165796510);d=md5_gg(d,a,b,c,x[i+6],9,-1069501632);
...
click();}});;$(window).on(SmartComments.Events.enums.OPEN_COMMENT_ID,function(data){var queryString=window.location.search;var urlParams=new URLSearchParams(queryString);var commentId=urlParams.get('focusId');if(mw.config.get('wgCanonicalSpecialPageName')==='SmartComments'){var commentId=data.pass;if($('div.sc-group-main').length!==0){SmartComments.Panels.CommentGroup.close();}SmartComments.Panels.CommentGroup.open(commentId,null,null,true);}else if(commentId){$('span[data-comment-id=\''+commentId+'\']').click();}});;var SmartComments=SmartComments||{};SmartComments.Api=SmartComments.Api||{};SmartComments.Api.Comments={mwApi:new mw.Api(),save:function(text){if(mw.config.get('wgCanonicalSpecialPageName')!='SmartComments'&&!this.isValid()){return{success:'0',message:'selection-error'};}var posString=typeof SmartComments.Selection.selection==="string"?SmartComments.Selection.selection:SmartComments.Selection.selection.text+"|"+SmartComments.Selection.selection.index,result=JSON.parse($.
ajax({type:'POST',url:mw.util.wikiScript('api')+'?action=smartcomments&method=new&format=json',data:{pos:posString,comment:typeof SmartComments.Selection.parent!=='undefined'?SmartComments.Selection.parent:'',text:text,page:mw.config.get('wgPageName'),image:SmartComments.Selection.image},async:false}).responseText);return typeof result.smartcomments==='undefined'?{success:'0',message:'api-error'}:result.smartcomments;},get:function(commentid,group,callback){this.mwApi.get({action:'smartcomments',method:'get',comment:commentid}).done(function(data){var result=(typeof data.smartcomments!=='undefined'&&typeof data.smartcomments.comment!=='undefined')?data.smartcomments.comment:null;callback(group,result);});},list:function(status,callback){this.mwApi.get({action:'smartcomments',method:'lista',page:mw.config.get('wgPageName'),status:status}).done(function(data){var result=typeof data.smartcomments!=='undefined'&&typeof data.smartcomments.anchors!=='undefined'?data.smartcomments.anchors:[];
callback(result);});},isValid:function(){return SmartComments.Selection.enabled||typeof SmartComments.Selection.selection==='undefined';},update:function(commentid,status,text,page){if(typeof page==='undefined'){page=mw.config.get('wgPageName');}var result=JSON.parse($.ajax({type:'POST',url:mw.util.wikiScript('api')+'?action=smartcomments&method=update&format=json',data:{comment:commentid,status:status,text:text,page:page},async:false}).responseText);return(typeof result.smartcomments==='undefined')?{success:'0',message:'api-error'}:result.smartcomments;},delete:function(commentid,page){if(typeof page==='undefined'){page=mw.config.get('wgPageName');}var result=JSON.parse($.ajax({type:'POST',url:mw.util.wikiScript('api')+'?action=smartcomments&method=delete&format=json',data:{comment:commentid,page:page},async:false}).responseText);return(typeof result.smartcomments==='undefined')?{success:'0',message:'api-error'}:result.smartcomments;},inBlockedMode:function(callback){this.mwApi.get({
action:'smartcomments',method:'blockedmode'}).done(function(data){var result=(typeof data.smartcomments!=='undefined'&&typeof data.smartcomments.message!=='undefined')?data.smartcomments.message:null;callback(result=='true');});}};;SmartComments.notifications={error:function(title,message){this.echo("error",title,message);},success:function(title,message){this.echo("success",title,message);},info:function(title,message){this.echo(null,title,message);},echo:function(type,title,text){VanillaToasts.create({title:title,text:text,type:type,timeout:mw.config.get("wgSmartComments").popupTimeout});}};SmartComments.popup={popupString:undefined,widget:undefined,close:function(visible){if(!visible){SmartComments.Selection.deselect(false);$('#sic-popup').remove();}},open:function(){this.widget.toggle(true);},create:function(x,y,linkElement){var self=this;if(typeof this.widget!=="undefined"){$('#sic-popup').remove();}this.widget=new OO.ui.PopupWidget({$content:$('<p>'+this.buttons(linkElement)+
'</p>'),padded:true,anchor:false,autoClose:true,width:'auto',id:'sic-popup'});this.widget.on('toggle',this.close);$(document.body).append(this.widget.$element);setTimeout(function(){$('#sic-popup').css({top:y,left:x});},10);return this;},buttons:function(linkElement){var newCommentButton=new OO.ui.ButtonWidget({label:mw.msg('sic-button-new-comment'),id:'sc-add-comment',icon:'ongoingConversation'});var linkButton=new OO.ui.ButtonWidget({label:mw.msg('sic-button-goto-link'),id:'sc-goto-link',icon:'link'});var button2=(typeof linkElement!=='undefined')?linkButton:false;var buttonGroup=new OO.ui.ButtonGroupWidget({items:(button2)?[newCommentButton,button2]:[newCommentButton]});return buttonGroup.$element.html();}};;var SmartComments=SmartComments||{};SmartComments.Buttons={bindEvents:function(){SmartComments.Buttons.AddComment.bindEvents();SmartComments.Buttons.EnableCommenting.bindEvents();SmartComments.Buttons.DisableCommenting.bindEvents();SmartComments.Buttons.CommentLinks.bindEvents()
;}};var SmartComments=SmartComments||{};SmartComments.Buttons=SmartComments.Buttons||{};SmartComments.Buttons.AddComment={bindEvents:function(){$(document).on('click','#sc-add-comment',function(e){SmartComments.Selection.deselect(false);SmartComments.Dialogs.NewCommentDialog.open();SmartComments.popup.close();e.preventDefault();});}};;var SmartComments=SmartComments||{};SmartComments.Buttons=SmartComments.Buttons||{};SmartComments.Buttons.EnableCommenting={bindEvents:function(){$(document).on('click','.sic-enable-commenting',function(e){$('.sic-enable-commenting a').parent().addClass('active selected');$('.sic-enable-commenting').removeClass('sic-enable-commenting').addClass('sic-disable-commenting');$(this).find('a').addClass('active');SmartComments.enableCommentMode(e);e.preventDefault();});}};;var SmartComments=SmartComments||{};SmartComments.Buttons=SmartComments.Buttons||{};SmartComments.Buttons.DisableCommenting={bindEvents:function(){$(document).on('click',
'.sic-disable-commenting',function(e){$('.sic-disable-commenting a').parent().removeClass('active selected');$('.sic-disable-commenting').removeClass('sic-disable-commenting').addClass('sic-enable-commenting');$(this).find('a').removeClass('active');SmartComments.disableCommentMode();e.preventDefault();});}};;var SmartComments=SmartComments||{};SmartComments.Buttons=SmartComments.Buttons||{};SmartComments.Buttons.CommentLinks={openComment:function(commentId,refElement,isBroken){if(SmartComments.enabled===true){if($('div.sc-group-main').length!==0){SmartComments.Panels.CommentGroup.close();}SmartComments.Panels.CommentGroup.open(commentId,'.smartcomment-hl-'+commentId,refElement,isBroken);}},bindEvents:function(){var self=this;$(document).on('click','*[class*=smartcomment-hl-]',function(e){var $class=$(this).attr('class'),$id=SmartComments.helperFunctions.getIdFromClass($class);self.openComment($id,$(this));if(SmartComments.enabled===true){e.preventDefault();}});}};;var SmartComments=
SmartComments||{};SmartComments.Dialogs={destruct:function(dialog){SmartComments.Selection.deselect();if(typeof dialog!=='undefined'){dialog.close();}}};var SmartComments=SmartComments||{};SmartComments.Dialogs=SmartComments.Dialogs||{};SmartComments.Dialogs.Actions=SmartComments.Dialogs.Actions||{};SmartComments.Dialogs.Actions.CommentSaveProcess={minCommentLength:1,doCancel:function(dialog){SmartComments.Dialogs.destruct(dialog);},doSave:function(dialog,commentText){var self=this;return new OO.ui.Process(function(){if(commentText.length<self.minCommentLength){return typeof dialog==="undefined"?mw.msg('sic-error-empty'):new OO.ui.Error(mw.msg('sic-error-empty'));}else{var result=SmartComments.Api.Comments.save(commentText);if(result.success=="1"||result.success===true){if(typeof dialog!=="undefined"){dialog.close();SmartComments.notifications.success(mw.msg('sic-added-comment'),mw.msg('sic-added-comment-refreshing'));setTimeout(function(){var parser=new URL(window.location);parser.
searchParams.set('action','screfresh');window.location=parser.href;},2000);}if(typeof dialog==="undefined"){return true;}}else if(typeof result.message!=='undefined'){return typeof dialog==="undefined"?result.message:new OO.ui.Error(result.message);}else{return typeof dialog==="undefined"?'Unknown error':new OO.ui.Error("Undefined error happened");}}});}};var SmartComments=SmartComments||{};SmartComments.Dialogs=SmartComments.Dialogs||{};SmartComments.Dialogs.NewCommentDialog={open:function(){function commentDialog(config){commentDialog.super.call(this,config);}OO.inheritClass(commentDialog,OO.ui.ProcessDialog);commentDialog.static.name='newComment';commentDialog.static.title=mw.msg('sic-title-new');commentDialog.static.actions=[{label:mw.msg('sic-button-save'),action:'save',flags:['primary','progressive']},{flags:'safe',action:'cancel',label:mw.msg('sic-button-cancel')}];commentDialog.prototype.getBodyHeight=function(){return 235;};commentDialog.prototype.initialize=function(){
commentDialog.super.prototype.initialize.call(this);this.panel=new OO.ui.PanelLayout({padded:true,expanded:false});this.content=new OO.ui.FieldsetLayout();this.commentInput=new OO.ui.MultilineTextInputWidget({value:'',autosize:true,maxRows:8});this.field=new OO.ui.FieldLayout(this.commentInput,{label:mw.msg('sic-input-newcomment'),align:'top'});this.content.addItems([this.field]);this.panel.$element.prepend($('<img class="sic-selected-content" src="'+SmartComments.Selection.image+'" />'));this.panel.$element.append(this.content.$element);this.$body.append(this.panel.$element);this.commentInput.connect(this,{'change':'onCommentInputChange'});};commentDialog.prototype.onCommentInputChange=function(value){this.actions.setAbilities({save:value.length>0&&value.length});};commentDialog.prototype.getActionProcess=function(action){if(action==='cancel'){SmartComments.Dialogs.Actions.CommentSaveProcess.doCancel(this);}else if(action==='save'){return SmartComments.Dialogs.Actions.
CommentSaveProcess.doSave(this,this.commentInput.getValue());}return commentDialog.super.prototype.getActionProcess.call(this,action);};commentDialog.prototype.getTeardownProcess=function(data){return commentDialog.super.prototype.getTeardownProcess.call(this,data).first(function(){$('#sc-wm').remove();},this);};var windowManager=new OO.ui.WindowManager({id:'sc-wm'});$(document.body).append(windowManager.$element);var myDialog=new commentDialog();windowManager.addWindows([myDialog]);windowManager.openWindow(myDialog);}};;var SmartComments=SmartComments||{};SmartComments.Panels={bindEvents:function(){}};var SmartComments=SmartComments||{};SmartComments.Panels=SmartComments.Panels||{};SmartComments.Panels.CommentGroup={refElement:null,active:false,isBroken:false,replies:[],anchor:undefined,last:{referringElement:undefined},enums:{ACTION_CLOSED:0,ACTION_COMPLETED:1,ACTION_DELETED:2},getBaseSelector:function(){if(SmartComments.activeSkin==='vector'){return SmartComments.bodyContainer.
parent().parent();}else if(SmartComments.activeSkin==='rhswiki'){return SmartComments.bodyContainer.parent().parent().parent();}else if(SmartComments.activeSkin==='gemmaonline'){return SmartComments.bodyContainer;}else{return SmartComments.bodyContainer;}},templating:{group:$($.parseHTML('<div class="sc-group-main"><!--<div class="smartcomment-overlay"></div>--><div class="smartcomment-panel"></div></div>')),comment:$($.parseHTML('<span><div class="meta"><h5>$AUTHOR$</h5><span class="date">$DATE$</span></div><div class="comment"><p>$TEXT$</p></div></span>'))},close:function(action){try{this.refElement=null;this.setActive(false);this.last.referringElement.removeClass("sc-active");this.refElement=null;$('.smartcomment-panel').empty();$('div.sc-group-main').remove();if(action===this.enums.ACTION_COMPLETED){SmartComments.notifications.success(mw.msg('sic-popup-completed'),mw.msg('sic-added-comment-refreshing'));}else if(action===this.enums.ACTION_DELETED){SmartComments.notifications.
success(mw.msg('sic-popup-deleted'),mw.msg('sic-added-comment-refreshing'));}else if(action===this.enums.ACTION_CLOSED){SmartComments.notifications.success(mw.msg('sic-popup-closed'),mw.msg('sic-added-comment-refreshing'));}}catch(e){}},create:function(comment){this.anchor=SmartComments.Highlighting.anchors[comment.id];var baseSelector=this.getBaseSelector();var that=this;var toolFactory=new OO.ui.ToolFactory();var toolGroupFactory=new OO.ui.ToolGroupFactory();var toolbar=new OO.ui.Toolbar(toolFactory,toolGroupFactory);var includes=[];function closeButton(){closeButton.parent.apply(this,arguments);}OO.inheritClass(closeButton,OO.ui.Tool);closeButton.static.name='close';closeButton.static.icon='close';closeButton.static.title=mw.msg('sic-button-close');closeButton.prototype.onSelect=function(){SmartComments.Panels.CommentGroup.close();};closeButton.prototype.onUpdateState=function(){};toolFactory.register(closeButton);includes.push('close');if(!mw.config.get('wgSmartComments').
directComment){function replyButton(){replyButton.parent.apply(this,arguments);}OO.inheritClass(replyButton,OO.ui.Tool);replyButton.static.name='newComment';replyButton.static.icon='speechBubbleAdd';replyButton.static.title=mw.msg('sic-button-new-reply');replyButton.prototype.onSelect=function(){SmartComments.Panels.CommentGroup.createReply(comment.id);this.setActive(false);};replyButton.prototype.onUpdateState=function(){};toolFactory.register(replyButton);includes.push('newComment');}if(comment.author===mw.config.get('wgUserName')||SmartComments.user.canManageComments()){function completeButton(){completeButton.parent.apply(this,arguments);}OO.inheritClass(completeButton,OO.ui.Tool);completeButton.static.name='complete';completeButton.static.icon='check';completeButton.static.title=mw.msg('sic-button-complete');completeButton.prototype.onSelect=function(){SmartComments.Panels.CommentGroup.complete(comment.id);this.setActive(false);};completeButton.prototype.onUpdateState=function(){}
;toolFactory.register(completeButton);includes.push('complete');}if(comment.author===mw.config.get('wgUserName')||SmartComments.user.canManageComments()){function deleteButton(){deleteButton.parent.apply(this,arguments);}OO.inheritClass(deleteButton,OO.ui.Tool);deleteButton.static.name='delete';deleteButton.static.icon='trash';deleteButton.static.title=mw.msg('sic-button-delete');deleteButton.prototype.onSelect=function(){SmartComments.Panels.CommentGroup.delete(comment.id);this.setActive(false);};deleteButton.prototype.onUpdateState=function(){};toolFactory.register(deleteButton);includes.push('delete');}if(SmartComments.user.canManageComments()){function overviewButton(){overviewButton.parent.apply(this,arguments);}OO.inheritClass(overviewButton,OO.ui.Tool);overviewButton.static.name='overview';if(mw.config.get('wgCanonicalSpecialPageName')==='SmartComments'){overviewButton.static.icon='article';overviewButton.static.title=mw.msg('sic-button-see-page');}else{overviewButton.static.
icon='articles';overviewButton.static.title=mw.msg('sic-button-overview');}overviewButton.prototype.onSelect=function(){SmartComments.Panels.CommentGroup.goToOverview(comment.id,comment.page);this.setActive(false);};overviewButton.prototype.onUpdateState=function(){};toolFactory.register(overviewButton);includes.push('overview');}toolbar.setup([{type:'bar',include:includes}]);var commentGroupFrame=new OO.ui.PanelLayout({expanded:false,framed:true,classes:['smartcomment-commentgroup'],id:'sc-commentgroup-'+comment.id});var contentFrame=new OO.ui.PanelLayout({expanded:false,padded:true,classes:['container sc-plh']});if(this.isBroken){contentFrame.$element.append(new OO.ui.PanelLayout({expanded:false,padded:false,$content:$('<img src="'+comment.positionImage+'" />'),classes:['sc-broken']}).$element);}contentFrame.$element.append(new OO.ui.PanelLayout({expanded:false,padded:false,$content:this.parseComment(comment),classes:['smartcomment-comment','comment-type-anchor']}).$element);var
lastAuthor=comment.modifiedBy;$.each(comment.replies,function(k,reply){contentFrame.$element.append(new OO.ui.PanelLayout({expanded:false,padded:true,$content:that.parseComment(reply),classes:['smartcomment-comment-reply']}).$element);lastAuthor=reply.modifiedBy;});if(mw.config.get('wgSmartComments').directComment&&lastAuthor!==mw.user.getName()){SmartComments.Api.Comments.inBlockedMode(function(inBlockedMode){if(!inBlockedMode){contentFrame.$element.append(SmartComments.Panels.InlineReply.create(contentFrame).$element);}});}commentGroupFrame.$element.append(toolbar.$element,contentFrame.$element);baseSelector.before().append(this.templating.group);baseSelector.find('div.smartcomment-panel').append(commentGroupFrame.$element);commentGroupFrame.$element.animate({left:"0px"},250);var top=$(window).scrollTop();if(this.anchor&&typeof this.anchor.elementPos!=="undefined"){top=this.anchor.elementPos.top-100;}else if(this.refElement!==null){top=this.refElement.position().top;}baseSelector.
find('div.smartcomment-panel').css({top:top,'max-height':"calc(100% - "+(this.isBroken?0:top)+'px)'});this.setActive(true);},complete:function(commentid){if(SmartComments.Api.Comments.update(commentid,'completed','')){this.close(this.enums.ACTION_COMPLETED);setTimeout(function(){var parser=new URL(window.location);parser.searchParams.set('action','screfresh');window.location=parser.href;},1500);}else{SmartComments.notifications.error('Error!','Couldnt close comment thread');}},delete:function(commentid){if(SmartComments.Api.Comments.delete(commentid)){this.close(this.enums.ACTION_DELETED);setTimeout(function(){var parser=new URL(window.location);parser.searchParams.set('action','screfresh');window.location=parser.href;},1500);}else{SmartComments.notifications.error('Error!','Couldnt close comment thread');}},edit:function(comment){var $anchorElement=$('div.smartcomment-panel div.comment-type-anchor');var commentTextField=$anchorElement.find('div.comment');commentTextField.html(
SmartComments.comment.forms.inlineEditPanel(commentTextField.text(),commentTextField).$element.html());},goToOverview:function(commentId,pageTitle){if(mw.config.get('wgCanonicalSpecialPageName')==='SmartComments'){var commentPage=new mw.Title(pageTitle);location.assign(commentPage.getUrl({'scenabled':'1','focusId':commentId}));}else{var pageName=mw.config.get('wgPageName');var scSpecial=new mw.Title('SmartComments',-1);var redirectUrl=mw.config.get('wgServer')+scSpecial.getUrl({'page':pageName});location.assign(redirectUrl);}},open:function(anchorId,elementClass,refElement,isBroken){$(elementClass).addClass("sc-active");this.isBroken=isBroken;this.last.referringElement=$(elementClass).css({'z-index':100,'position':'relative'});SmartComments.Api.Comments.get(anchorId,this,function(group,comment){SmartComments.Events.trigger(SmartComments.Events.enums.COMMENT_GROUP_OPEN);SmartComments.Selection.setSelectionFromString(comment.position);SmartComments.Selection.parent=comment.id;if(!
isBroken){group.refElement=refElement;}group.create(comment);});},parseComment:function(comment,antiXss){antiXss=typeof antiXss!=='undefined'?antiXss:false;return this.templating.comment.html().replace('$AUTHOR$',comment.author).replace('$DATE$',comment.datetime).replace('$TEXT$',antiXss?SmartComments.helperFunctions.encodeHTML(comment.text):$(comment.text).text());},addReply:function(text){var replyFrame=new OO.ui.PanelLayout({expanded:false,padded:true,$content:this.parseComment({datetime:mw.msg('sic-date-justnow'),text:text,author:mw.config.get('wgUserName')}),classes:['smartcomment-comment-reply']}).$element;$('div.sc-plh').append(replyFrame);},setActive:function(active){this.active=active;}};var SmartComments=SmartComments||{};SmartComments.Panels=SmartComments.Panels||{};SmartComments.Panels.InlineReply={create:function(contentFrame){this.panel=new OO.ui.PanelLayout({padded:true,expanded:false,classes:['smartcomment-reply']});this.content=new OO.ui.FieldsetLayout();this.
commentInput=new OO.ui.MultilineTextInputWidget({value:'',maxRows:5,autosize:true});this.commentSaveButton=new OO.ui.ButtonWidget({label:mw.msg('sic-button-new-reply')});var self=this;var wasSubmittedBefore=false;this.commentSaveButton.on('click',function(){if(wasSubmittedBefore){return false;}var commentText=self.commentInput.getValue(),wasSaved=SmartComments.Dialogs.Actions.CommentSaveProcess.doSave(undefined,commentText).steps[0].callback();if(wasSaved!==true){SmartComments.notifications.error(mw.msg('sic-error-title'),wasSaved);}else{contentFrame.$element.append(new OO.ui.PanelLayout({expanded:false,padded:true,$content:SmartComments.Panels.CommentGroup.parseComment({author:mw.config.get('wgUserName'),datetime:mw.msg('sic-date-justnow'),text:commentText},true),classes:['smartcomment-comment-reply']}).$element);self.commentInput.setValue("");$('div.smartcomment-panel div.smartcomment-commentgroup div.smartcomment-reply').appendTo('div.sc-plh');wasSubmittedBefore=true;$(
'.smartcomment-reply').remove();}});this.fieldComment=new OO.ui.FieldLayout(this.commentInput,{label:mw.msg('sic-input-commenttext'),align:'top'});this.fieldSaveButton=new OO.ui.FieldLayout(this.commentSaveButton);this.content.addItems([this.fieldComment,this.fieldSaveButton]);this.panel.$element.append(this.content.$element);return this.panel;}};var SmartComments=SmartComments||{};SmartComments.Panels=SmartComments.Panels||{};SmartComments.Panels.CommentTimeline={supportedSkins:['vector','gemmaonline','rhswiki'],positions:[],idsHandled:[],templates:{base:$('<div class="sic-timeline-container" />'),item:$('<div class="sic-timeline-item"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="13" fill="currentColor" class="bi bi-chat-right-text" viewBox="0 0 16 12"><path d="M3 3.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5zM3 6a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 3 6zm0 2.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5z"/></svg></div>'),hover:$(
'<div class="sic-timeline-item-hover" />')},skinSelectors:{vector:$('.mw-body'),gemmaonline:$('section#mainContent'),rhswiki:$('main#content'),getSelector:function(){if(SmartComments.activeSkin==='gemmaonline'){return this.gemmaonline;}else if(SmartComments.activeSkin==='rhswiki'){return this.rhswiki;}else{return this.vector;}},getMargin:function(){if(SmartComments.activeSkin==='gemmaonline'){return 135;}else if(SmartComments.activeSkin==='rhswiki'){return 115;}else{return 80;}}},isSkinSupported:function(){return SmartComments.Panels.CommentTimeline.supportedSkins.indexOf(SmartComments.activeSkin)!==-1;},createBase:function(){this.templates.base.appendTo(SmartComments.getNodeRoot(true)).css({top:-this.skinSelectors.getSelector().position().top-this.skinSelectors.getMargin()});this.bindEvents();},addToBase:function(comment){var self=this,id=typeof comment.data_id!=="undefined"?comment.data_id:comment;if(this.idsHandled.indexOf(id)!==-1){return;}setTimeout(function(){var item=self.
templates.item.clone(),ref=$('.smartcomment-hl-'+id),positionTop=0,isBroken=ref.length===0;if(SmartComments.activeSkin==='rhswiki'){positionTop=$('main.mainContainer').height()+SmartComments.Panels.CommentTimeline.skinSelectors.getMargin()+120;}else{positionTop=$('#bodyContent').height()+90;}if(!isBroken){positionTop=typeof comment.elementPos!=="undefined"?comment.elementPos.top:ref.offset().top;}else{item.addClass('broken');item.attr('data-selection',comment.posimg);}if(self.positions.indexOf(positionTop)!==-1){positionTop+=5;}if(comment.hasReplies==='true'){item.addClass('sic-timeline-item-hasreplies');}item.css({top:positionTop,right:-3,position:'absolute'});item.appendTo(self.templates.base);item.on('mouseover',function(){ref.addClass('active');}).on('mouseout',function(){ref.removeClass('active');}).on('click',function(){SmartComments.Buttons.CommentLinks.openComment(id,isBroken?item:ref,isBroken);});self.positions.push(positionTop);self.idsHandled.push(id);},1);},bindEvents:
function(){$(document).on('mouseover','.sic-timeline-item.broken',function(e){var selection=$(this).data('selection'),hover=SmartComments.Panels.CommentTimeline.templates.hover.clone(),img=$('<img src="'+selection+'" width="500" />');if(typeof selection!=="undefined"){hover.append('<h2>'+mw.msg("sic-unlocalized-comment")+'</h2>');hover.append(img);hover.css({position:'absolute',right:"30px",top:e.pageY-50,border:"2px solid #ff000050"});$('body').append(hover);}}).on('mouseout','.sic-timeline-item.broken',function(){$('.sic-timeline-item-hover').each(function(){$(this).remove();});});},destruct:function(){$('.sic-timeline-container').remove();}};;var SmartComments=SmartComments||{};SmartComments.Highlighting={highlighter:rangy.createHighlighter(),enums:{HIGHLIGHT_CLASS:'sic-highlighted-item'},comments:[],anchors:[],Types:{Callers:{main:[],addon:[]},Enums:{MAIN:'main',ADDON:'addon'},Register:function(type,handler){this.Callers[type].push(handler);}},bindEvents:function(){this.highlighter
.addClassApplier(rangy.createClassApplier(this.enums.HIGHLIGHT_CLASS,{ignoreWhiteSpace:true}));},destruct:function(){for(var i=0;i<this.Types.Callers[this.Types.Enums.ADDON].length;i++){this.Types.Callers[this.Types.Enums.ADDON][i].destruct();}$.each(this.comments,function(k,comment){if(comment){$('*.smartcomment-hl-'+comment.data_id).removeClass('smartcomment-hl-'+comment.data_id).removeAttr('data-comment-id');}});SmartComments.Panels.CommentTimeline.destruct();},loadFromCommentsOnPage:function(commentsOnPage){this.comments=commentsOnPage;if(SmartComments.Panels.CommentTimeline.isSkinSupported()){SmartComments.Panels.CommentTimeline.createBase();}for(var i=0;i<commentsOnPage.length;i++){var comment=this.highlightOnPage(commentsOnPage[i]);if(SmartComments.Panels.CommentTimeline.isSkinSupported()){SmartComments.Panels.CommentTimeline.addToBase(commentsOnPage[i]);}this.anchors[comment.data_id]=comment;}},highlightOnPage:function(comment){for(var i=0;i<this.Types.Callers[this.Types.Enums.
MAIN].length;i++){var caller=this.Types.Callers[this.Types.Enums.MAIN][i].init(comment);if(caller.isCorrectType()){return caller.highlight();}}for(var i=0;i<this.Types.Callers[this.Types.Enums.ADDON].length;i++){var caller=this.Types.Callers[this.Types.Enums.ADDON][i].init(comment);if(caller.isCorrectType()){comment.pos=comment.pos.split('%')[1];return caller.highlight();}}return false;},addDataAttributesToElement:function(element,comment){element.attr('data-comment-id',comment.data_id);}};;var SmartComments=SmartComments||{};var highlighter=SmartComments.Highlighting=SmartComments.Highlighting||{};SmartComments.Highlighting.Types.WordIndex={comment:undefined,highlighter:rangy.createHighlighter(),init:function(comment){this.comment=comment;return this;},isCorrectType:function(){return!/<[^>]*>/.test(this.comment.pos.replace(/&lt;/g,'<').replace(/&gt;/g,'>'))&&this.comment.pos.indexOf('|')!==-1;},highlight:function(){var pos=this.comment.pos.split('|'),word=pos[0],index=parseInt(pos[1])
,baseEl=SmartComments.getNodeRoot(),range=rangy.createRange(),searchScopeRange=rangy.createRange(),i=0;if(index===-1){return false;}range.selectNodeContents(baseEl);searchScopeRange.selectNodeContents(baseEl);var options={caseSensitive:true,withinRange:searchScopeRange};while(range.findText(word,options)){if(i===index){this.highlightRange(range,this.comment);return this.comment;}range.collapse(false);++i;}return false;},highlightRange:function(range,comment,commentClassName){commentClassName=typeof commentClassName==="undefined"?'smartcomment-hl-'+comment.data_id:commentClassName;var sel=rangy.getSelection();sel.setSingleRange(range);this.highlighter.addClassApplier(rangy.createClassApplier(commentClassName,{ignoreWhiteSpace:true}));this.highlighter.highlightSelection(commentClassName,{selection:sel});sel.removeAllRanges();if(comment){SmartComments.Highlighting.addDataAttributesToElement($('.smartcomment-hl-'+comment.data_id),comment);}}};highlighter.Types.Register(highlighter.Types.
Enums.MAIN,SmartComments.Highlighting.Types.WordIndex);;var SmartComments=SmartComments||{};var highlighter=SmartComments.Highlighting=SmartComments.Highlighting||{};highlighter.Types.Register(highlighter.Types.Enums.MAIN,{comment:undefined,init:function(comment){this.comment=comment;return this;},isCorrectType:function(){return this.comment.pos.indexOf('[')!==-1&&this.comment.pos.indexOf(']')!==-1;},highlight:function(){this.comment.pos=SmartComments.helperFunctions.decodeHTML(this.comment.pos);var element=undefined,isImage=false;if(this.comment.pos.indexOf('img')!==-1){element=this.determineImageLocation(this.comment.pos);isImage=true;}else{element=$(this.comment.pos);}if(this.comment&&element){if(isImage){element.parent().addClass('smartcomment-hl-'+this.comment.data_id);}else{element.addClass('smartcomment-hl-'+this.comment.data_id);}SmartComments.Highlighting.addDataAttributesToElement($('.smartcomment-hl-'+this.comment.data_id),this.comment);}return this.comment;},
determineImageLocation(hash){hash=hash.replace('img[','').replace(']','');var element=null;SmartComments.bodyContainer.find('img').each(function(){var imageHash=SmartComments.Selection.ImageSelection.getHash($(this));if(hash===imageHash){element=$(this);}});return element;}});;var SmartComments=SmartComments||{};var highlighter=SmartComments.Highlighting=SmartComments.Highlighting||{};SmartComments.Highlighting.Types.FindSelectionIndex={comment:undefined,init:function(comment){this.comment=comment;return this;},isCorrectType:function(){return/<[^>]*>/.test(this.comment.pos.replace(/&lt;/g,'<').replace(/&gt;/g,'>'))&&this.comment.pos.indexOf('|')!==-1;},highlight:function(){var pos=this.comment.pos.split('|'),word=pos[0].replace(/&lt;/g,'<').replace(/&gt;/g,'>'),index=parseInt(pos[1]),baseEl=SmartComments.getNodeRoot(),range=rangy.createRange(),searchScopeRange=rangy.createRange(),i=0;if(index===-1){return false;}range.selectNodeContents(baseEl);searchScopeRange.selectNodeContents(
baseEl);var options={caseSensitive:true,withinRange:searchScopeRange};word=word.replace(/(<([^>]+)>)/gi,"");while(range.findText(word,options)){if(i===index){this.highlightRange(range,this.comment);return this.comment;}range.collapse(false);++i;}return false;},highlightRange:function(range,comment,commentClassName){commentClassName=typeof commentClassName==="undefined"?'smartcomment-hl-'+comment.data_id:commentClassName;var span=document.createElement('span');span.className=commentClassName;span.setAttribute('data-comment-id',comment.data_id);try{span.appendChild(range.extractContents());}catch(error){console.error('Error extracting contents:',error);return;}range.insertNode(span);var sel=rangy.getSelection();sel.removeAllRanges();if(comment){SmartComments.Highlighting.addDataAttributesToElement($(span),comment);}}};highlighter.Types.Register(highlighter.Types.Enums.MAIN,SmartComments.Highlighting.Types.FindSelectionIndex);;var SmartComments=SmartComments||{};SmartComments.Selection={
enabled:false,selection:undefined,parent:undefined,image:undefined,enums:{SELECTION_VALID:0,INVALID_SELECTION_ALREADY_COMMENTED:1,INVALID_SELECTION_INCLUDES_DYNAMIC_CONTENT:2,INVALID_SELECTION_CONTAINS_LINEBREAKS:3,INVALID_SELECTION_IS_EMPTY:4},lastPos:{x:0,y:0},startPos:{x:0,y:0},lastRange:undefined,addons:[],isCapturing:false,executeSelectionAction:function(selection){console.warn('SmartComments.Selection.executeSelectionAction is deprecated');console.warn('Use SmartComments.Selection.preSelection and SmartComments.Selection.postSelection instead');var self=this;this.selection=selection;SmartComments.helperFunctions.toggleSpinner(this.lastPos.x,this.lastPos.y);this.screenshotSelection(function(canvasBase64){self.image=canvasBase64;self.parent=undefined;SmartComments.popup.create((SmartComments.Selection.lastPos.x+5),(SmartComments.Selection.lastPos.y+5)).open();SmartComments.helperFunctions.toggleSpinner();SmartComments.Events.trigger(SmartComments.Events.enums.SELECTION_ACTIVE);});}
,preSelection:function(range){SmartComments.Selection.lastRange=range;SmartComments.helperFunctions.toggleSpinner(this.lastPos.x,this.lastPos.y);this.isCapturing=true;},postSelection:function(selection){var self=this;this.selection=selection;SmartComments.Highlighting.Types.WordIndex.highlightRange(this.lastRange,null,SmartComments.Highlighting.enums.HIGHLIGHT_CLASS);this.screenshotSelection(function(canvasBase64){self.image=canvasBase64;self.parent=undefined;SmartComments.popup.create((self.startPos.x+5),(self.startPos.y+5)).open();SmartComments.helperFunctions.toggleSpinner();SmartComments.Events.trigger(SmartComments.Events.enums.SELECTION_ACTIVE);$('.'+SmartComments.Highlighting.enums.HIGHLIGHT_CLASS).removeClass(SmartComments.Highlighting.enums.HIGHLIGHT_CLASS);self.isCapturing=false;});},deselect:function(resetState){rangy.getSelection().removeAllRanges();resetState=typeof resetState==='undefined'?true:resetState;SmartComments.Selection.DynamicBlockSelection.deselect();if(
resetState){this.lastPos={x:0,y:0};this.selection=undefined;this.parent=undefined;SmartComments.Selection.image=undefined;$('.sic-canvas').remove();}},validateSelection:function(wrappedSelection){var selectionHTML=typeof wrappedSelection==='string'?wrappedSelection:wrappedSelection.toHtml(),res=this.enums.SELECTION_VALID;if(selectionHTML.indexOf('smartcomment-hl-')!==-1){res=this.enums.INVALID_SELECTION_ALREADY_COMMENTED;}else if(selectionHTML.indexOf('sc-dynamic-block')!==-1){res=this.enums.INVALID_SELECTION_INCLUDES_DYNAMIC_CONTENT;}else if(selectionHTML.trim()===''){res=this.enums.INVALID_SELECTION_IS_EMPTY;}else if(selectionHTML.match(/[\n\r]/)){res=this.enums.INVALID_SELECTION_CONTAINS_LINEBREAKS;}if(res!==this.enums.SELECTION_VALID){this.showError(res);}return res;},bindEvents:function(){SmartComments.Selection.ImageSelection.bindEvents();SmartComments.Selection.TextSelection.bindEvents();SmartComments.Selection.DynamicBlockSelection.bindEvents();var self=this;$(document.body).
mousemove(function(event){if(self.isCapturing){return;}self.lastPos.x=event.pageX;self.lastPos.y=event.pageY;});},showError:function(id){SmartComments.notifications.error(mw.msg('sic-selection-error-title'),mw.msg('sic-selection-error-'+id));},setSelectionFromString:function(position){if(position.indexOf("|")!==-1){var parts=position.split("|");this.selection={text:parts[0],index:parts[1]};}else this.selection=position;},screenshotSelection:function(callback){var minMaxWidth=500,minMaxHeight=50,maxChars=45,width=this.lastPos.x-this.startPos.x,height=this.lastPos.y-this.startPos.y,scale=100;if(width<minMaxWidth||width>minMaxWidth){width=minMaxWidth;}if(height<minMaxHeight||height>minMaxHeight){height=minMaxHeight;}var x=this.lastPos.x-(this.lastPos.x-this.startPos.x)/2,y=this.lastPos.y-(this.lastPos.y-this.startPos.y)/2;if(typeof SmartComments.Selection.selection!=='undefined'&&typeof SmartComments.Selection.selection.text!=='undefined'&&SmartComments.Selection.selection.text.length>
maxChars){scale=(100*maxChars)/SmartComments.Selection.selection.text.length;}SmartComments.helperFunctions.screenshot("default",{x:x-width/2,y:y-height/2,width:minMaxWidth,height:minMaxHeight,onclone:function(clone){var activeItems=clone.getElementsByClassName(SmartComments.Highlighting.enums.HIGHLIGHT_CLASS);for(var i=0;i<activeItems.length;i++){activeItems[i].style.background="#ffffe0";activeItems[i].style["border-top"]="1px solid rgba(0,0,0,0.2)";activeItems[i].style["border-bottom"]="1px solid rgba(0,0,0,0.2)";if(i===0){activeItems[i].style["border-left"]="1px solid rgba(0,0,0,0.2)";}if(i===activeItems.length-1){activeItems[i].style["border-right"]="1px solid rgba(0,0,0,0.2)";}}clone.getElementById('sic-spinner').remove();$(clone).find('image').remove();return clone;}},function(canvas){callback(canvas);});},registerAddon:function(name,addon){if(SmartComments.enabledAddons.indexOf(name)!==-1){addon.bindEvents();}}};;var SmartComments=SmartComments||{};SmartComments.Selection=
SmartComments.Selection||{};SmartComments.Selection.TextSelection={active:false,select:function(event){if(this.active){return;}this.active=true;var selection=rangy.getSelection(SmartComments.getNodeRoot()),self=this;if(SmartComments.Selection.validateSelection(selection)===SmartComments.Selection.enums.SELECTION_VALID){var range=selection.getRangeAt(0);if(!range.endContainer.parentNode.isSameNode(range.startContainer.parentNode)&&!range.endContainer.parentNode.contains(range.startContainer.parentNode)){range.setEnd(range.endContainer,range.endContainer.length);}if(!range.startContainer.parentNode.contains(range.endContainer.parentNode)){range.setStart(range.startContainer,0);}SmartComments.Selection.preSelection(range);this.getTextAndIndexAsync(range).then(result=>{SmartComments.Selection.postSelection(result);self.active=false;}).catch((e)=>{self.active=false;console.log("Exception caught",e);});}else{self.active=false;}},getTextAndIndex:function(selection){var baseEl=SmartComments.
getNodeRoot(),selectionPos=rangy.serializeSelection(selection),searchFor=selection.toString(),range=rangy.createRange(),searchScopeRange=rangy.createRange(),i=0;range.selectNodeContents(baseEl);searchScopeRange.selectNodeContents(baseEl);var options={caseSensitive:true,withinRange:searchScopeRange},res={text:searchFor,index:i,range:selectionPos}
