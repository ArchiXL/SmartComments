...
use ApiBase, Title;
use MWException;
use MediaWiki\Extension\SmartComments\DBHandler as DBHandler;
use MediaWiki\Extension\SmartComments\Settings\Handler;
use MediaWiki\Extension\SmartComments\SMWHandler as SMWHandler;
...
class Api extends ApiBase {
	...
	private function doUpdate() {
		...
		$text = $this->getRequest()->getVal(self::PARAM_TEXT, '');
		if (is_null($commentId) || !(empty($text) xor empty($status)) ||
	...
	private function doListAnchors() {
		...
		if (empty($pageName) || !in_array($filter, [SIC::STATUS_OPEN, SIC::STATUS_COMPLETED, ''])) {
			...
			if ( $rev !== '' ) {
				$anchors = array_filter( $anchors, function( $item ) use ( $rev ) {
